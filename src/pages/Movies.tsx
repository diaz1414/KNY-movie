...
const Movies: React.FC = () => {
  ...
  // Handle URL query parameters for movie detail
  useEffect(() => {
  ...
  useEffect(() => {
    const savedState = sessionStorage.getItem('movies_page_state');
    if (savedState) {
      const {
        movies: savedMovies,
  ...
  useEffect(() => {
    const handleSaveState = () => {
      if (movies.length > 0) {
        sessionStorage.setItem('movies_page_state', JSON.stringify({
          movies,
          heroMovies,
          page,
          hasMore,
    ...
    const handleScroll = () => {
      if (movies.length > 0) {
        const current = JSON.parse(sessionStorage.getItem('movies_page_state') || '{}');
        sessionStorage.setItem('movies_page_state', JSON.stringify({
          ...current,
          scrollY: window.scrollY
        }));
  ...
  const fetchInitialData = async () => {
    ...
    try {
      const [popular, recent] = await Promise.all([
        ...
        movieService.getRecentMovies(1)
      ]);
      setHeroMovies(popular.slice(0, 7));
      setMovies(recent);
      setPage(1);
      setHasMore(recent.length > 0);
    } catch (err) {
  ...
  const loadMore = async () => {
    ...
    try {
      const nextMovies = await movieService.getRecentMovies(nextPage);
