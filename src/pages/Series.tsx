...
const Series: React.FC = () => {
  ...
  // Handle URL query parameters for series detail
  useEffect(() => {
  ...
  useEffect(() => {
    const handleSaveState = () => {
      if (movies.length > 0) {
        sessionStorage.setItem('series_page_state', JSON.stringify({
          movies,
          heroMovies,
          page,
          hasMore,
    ...
    const handleScroll = () => {
      if (movies.length > 0) {
        const current = JSON.parse(sessionStorage.getItem('series_page_state') || '{}');
        sessionStorage.setItem('series_page_state', JSON.stringify({
          ...current,
          scrollY: window.scrollY
        }));
  ...
  const fetchInitialData = async () => {
    setLoading(true);
    try {
      const popular = await movieService.getPopularSeries(1);
  ...
  const loadMore = async () => {
    ...
    try {
      const nextSeries = await movieService.getPopularSeries(nextPage);
      if (nextSeries.length > 0) {
        setMovies((prev) => [...prev, ...nextSeries]);
        setPage(nextPage);
      } else {
        setHasMore(false);
      }
