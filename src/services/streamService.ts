...
export const getProxiedUrl = (url: string, force = false) => {
  ...
  const proxyBase = isLocal
    ? '/api/proxy'
    : (import.meta.env.VITE_PROXY_BASE_URL || 'https://proxy-ykntv414.ykn.my.id/api/proxy');
...
export const getCdnBackupUrl = (url: string, force = false) => {
  const cleanTargetUrl = normalizeStreamUrl(url);
  // Selalu pakai CDN backup eksternal (tv.ykn.my.id/api/proxy), termasuk saat localhost
...
const buildTvstreamTokenServers = (tokenUrl: string, jenis: string): StreamServer[] => {
  ...
  servers.push({
    name: getCdnBackupBaseUrl() ? 'Server 2 (SBS Token)' : 'Server 1 (SBS Token)',
    url: tokenUrl,
    type: jenis || 'hls',
    forceProxy: false,
    tokenChannelId,
...
export const buildServers = (
  ...
  // Server 1: Direct — langsung ke CDN sumber tanpa proxy
  servers.push({
  ...
  // Server 2: CDN Proxy — lewat proxy backup (tv.ykn.my.id atau /api/proxy saat localhost)
  servers.push({
    name: 'Server 2 (CDN Proxy)',
    url: rawUrl,
    type: jenis,
    forceProxy: false,
    cdnBackup: true,
  ...
  // Server 3 (opsional): Proxy lama untuk http:// atau forceProxy eksplisit
  if (rawUrl.startsWith('http://') || forceProxyFallback) {
    servers.push({
      name: `Server 3 (Proxy Backup)`,
      url: rawUrl,
      type: jenis,
      forceProxy: true,
    });
